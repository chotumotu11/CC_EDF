		...
		while(g[i].slack!=0 && head!=NULL){
			if(jobexecuting == NULL){
				...
				if(g[i].slack < jobexecuting->slack){
					...
					printf("\njobexecuting->actualstart=%f",jobexecuting->actualstart);
					printf("\nbef jobexecuting->slack=%f",jobexecuting->slack);
					jobexecuting->slack = (jobexecuting->slack)-g[i].slack;
					printf("\naf jobexecuting->slack-g[i].slack=%f",jobexecuting->slack);
					g[i].slack = 0;
					jobexecuting->slack = (jobexecuting->slack)*(freq/maxfreq);
					printf("\ng[%d].slack=%f",i,g[i].slack);
					printf("\njobexecuting->wctexe=%f",jobexecuting->wctexe);
					printf("\nend jobexecuting->slack=%f",jobexecuting->slack);
					...
					printf("\njobexecuting->actualstop=%f",jobexecuting->actualstop);
				...
				if((jobexecuting->deadline)>(head->deadline)){
					...
					printf("\ndl bef jobexecuting->slack=%f",jobexecuting->slack);
					//jobexecuting->slack+=(0.2+0.1+0.05);//preemption, decision making and freq calc overhead
					if(g[i].slack < jobexecuting->slack){
						jobexecuting->actualstart=i+(1-g[i].slack);
						printf("\njobexecuting->actualstart=%f",jobexecuting->actualstart);
						jobexecuting->slack = (jobexecuting->slack)-g[i].slack;
						printf("\ndl af jobexecuting->slack-g[i].slack=%f",jobexecuting->slack);
						g[i].slack = 0;
						jobexecuting->slack = (jobexecuting->slack)*(freq/maxfreq);
						printf("\ndl end jobexecuting->slack=%f",jobexecuting->slack);
						insertintog(jobexecuting,g+i);
					}else{
						jobexecuting->actualstart=i+(1-g[i].slack);
						printf("\njobexecuting->actualstart=%f",jobexecuting->actualstart);
					...
					printf("\ndl else bef jobexecuting->slack=%f",jobexecuting->slack);
					if(g[i].slack < jobexecuting->slack){
						jobexecuting->slack = (jobexecuting->slack)-g[i].slack;
						printf("\ndl else af jobexecuting->slack-g[i].slack=%f",jobexecuting->slack);
						g[i].slack = 0;
						jobexecuting->slack = (jobexecuting->slack)*(freq/maxfreq);
						printf("\ndl else end jobexecuting->slack=%f",jobexecuting->slack);
						insertintog(jobexecuting,g+i);
					}else{
						g[i].slack = g[i].slack-(jobexecuting->slack);
						jobexecuting->actualstop=i+jobexecuting->slack;
...
void printschedule(struct granularity *g,int bmax){
	...
	for(int i = 0;i<bmax;i++){
		...
		while(co!=NULL){
			printf("J%d %d %f\t",co->oftask,co->job , co->actualstop);
			co=co->next;
		}
		printf("\n");
	}
...
void printresponseTime(struct granularity *g,int bmax ,int numtask){
	...
	{
		fprintf(fptr,"Task=%d Job=%d arrival=%d exe=%f start=%f stop=%f resp=%f wctexe=%f\n",travers_temp->oftask,travers_temp->job,travers_temp->arrival,travers_temp->exe,travers_temp->actualstart , travers_temp->actualstop ,(travers_temp->actualstop-travers_temp->arrival),travers_temp->wctexe);
